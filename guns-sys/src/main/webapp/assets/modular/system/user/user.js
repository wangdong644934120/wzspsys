layui.use(['layer', 'form', 'table', 'ztree', 'laydate', 'admin', 'ax'], function () {
     ...
     * @param data 点击按钮时候的行数据
     */
    MgrUser.setProject = function (data) {
        layer.open({
            type: 2,
            title: '分配项目',
            area: ['300px', '400px'],
            content: Feng.ctxPath + '/mgr/project_assign?userId=' + data.userId,
            end: function () {
                table.reload(MgrUser.tableId);
            }
        });
    ...
    table.on('tool(' + MgrUser.tableId + ')', function (obj) {
        ...
        } else if (layEvent === 'setProject') {
            MgrUser.setProject(data);
        }
    });
