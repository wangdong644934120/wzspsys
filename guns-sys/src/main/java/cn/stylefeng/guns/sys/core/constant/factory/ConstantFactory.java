...
public class ConstantFactory implements IConstantFactory {
    ...
    @Override
    @Cacheable(value = Cache.CONSTANT, key = "'" + CacheKey.DICT_LIST + "'+#dictTypeId")
    public List<Dict> findByDictType(String dictTypeId){
        if (ToolUtil.isEmpty(dictTypeId)) {
            return null;
        } else {
            QueryWrapper<Dict> wrapper = new QueryWrapper<>();
            List<Dict> dicts = dictMapper.getDictListByType(dictTypeId);
    ...
    public HashMap<String,Dict> findByDictTypeMap(String dictTypeId){
        List<Dict> list=this.findByDictType(dictTypeId);
        HashMap<String,Dict> map=new HashMap<>();
        for(Dict dict : list){
            map.put(dict.getDictId(),dict);
        }
        return map;
    }
