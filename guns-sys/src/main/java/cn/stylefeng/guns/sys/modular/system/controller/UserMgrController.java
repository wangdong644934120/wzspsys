...
public class UserMgrController extends BaseController {
     ...
     * @Date 2018/12/24 22:43
     */
    @RequestMapping("/project_assign")
    public String projectAssign(@RequestParam Long userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        model.addAttribute("userId", userId);
        return PREFIX + "user_projectassign.html";
    }
...
//        }
    ...
    @BussinessLog(value = "分配项目", key = "userId,projectIds", dict = UserDict.class)
    @Permission(Const.ADMIN_NAME)
    @ResponseBody
    public ResponseData setProject(@RequestParam("userId") Long userId, @RequestParam("projectIds") String projectIds) {
        if (ToolUtil.isOneEmpty(userId, projectIds)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        //不能修改超级管理员
        if (userId.equals(Const.ADMIN_ID)) {
            throw new ServiceException(BizExceptionEnum.PROJECT_CHANGE_ADMIN);
        }
        this.userService.assertAuth(userId);
        this.userService.setProjects(userId, projectIds);
        return SUCCESS_TIP;
    }
     ...
     * @Date 2018/12/24 22:44
     */
    @RequestMapping("/setGovunit")
    @BussinessLog(value = "分配单位", key = "userId,govunitIds", dict = UserDict.class)
    @Permission(Const.ADMIN_NAME)
    @ResponseBody
    public ResponseData setGovunit(@RequestParam("userId") Long userId, @RequestParam("govunitIds") String govunitIds) {
        if (ToolUtil.isOneEmpty(userId, govunitIds)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        //不能修改超级管理员
        if (userId.equals(Const.ADMIN_ID)) {
        ...
        this.userService.assertAuth(userId);
        this.userService.setGovunits(userId, govunitIds);
        return SUCCESS_TIP;
    }
    ...
    public ResponseData menulist() {
        ...
        if (roleList == null || roleList.size() == 0) {
            return ResponseData.error("该用户没有角色");
        }
